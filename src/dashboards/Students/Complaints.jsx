...
const Complaints = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (!file.type.startsWith("image/")) {
      setError("Only image files are allowed for attachment.");
      setAttachmentFile(null);
      setAttachmentPreview(null);
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (attachmentFile) {
        const base64 = await readFileAsDataURL(attachmentFile);
        submitData.attachmentBase64 = base64;
      }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-1 bg-[#EEF8F3]/60 rounded-3xl p-6 lg:p-8 flex flex-col border border-emerald-50 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-emerald-800 uppercase tracking-widest mb-1.5 ml-1">
                Category
              </label>
              <select
            ...
            <div>
              ...
              <input
                ...
                className="w-full text-sm text-gray-700 file:mr-3 file:py-2 file:px-3 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-100 file:text-emerald-700 hover:file:bg-emerald-200"
              />
              {attachmentPreview && (
                <img
                  src={attachmentPreview}
                  alt="Attachment preview"
                  className="mt-3 max-h-36 rounded-xl border border-gray-200 object-cover"
                />
              )}
            </div>
          ...
          <div className="flex flex-col gap-4">
            {complaints.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                No complaints yet. Use the form above to register your first
                complaint.
              </div>
            ) : (
              complaints.map((comp) => {
                ...
                return (
                  <div
                    key={comp.id}
                    className="bg-white rounded-2xl p-5 shadow-sm border border-transparent hover:border-sky-100 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group relative"
                  >
                    <button
                      onClick={async () => {
                        if (window.confirm('Are you sure you want to delete this complaint? This action cannot be undone.')) {
                          try {
                            await complaintApi.delete(`/student/complaints/${comp.id}`);
                            fetchComplaints();
                          } catch (err) {
                      ...
                      className="absolute top-3 right-3 p-1.5 bg-white/80 hover:bg-white rounded-lg shadow-md border hover:border-red-200 hover:shadow-lg transition-all opacity-0 group-hover:opacity-100 z-10 md:opacity-100 md:group-hover:opacity-100 text-red-500 hover:text-red-600"
                    ...
                    <div className="flex items-start gap-4">
                      <div
                        className={`p-3 rounded-xl mt-1 shadow-inner ${statusConfig.color === "orange" || statusConfig.color === "blue" ? `bg-${statusConfig.color}-100 text-${statusConfig.color}-500` : `bg-${statusConfig.color}-100 text-${statusConfig.color}-500`}`}
                      >
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span
                            className={`text-xs font-bold px-2.5 py-1 rounded-full ${comp.priority === "high" ? "text-red-600 bg-red-50" : comp.priority === "medium" ? "text-orange-600 bg-orange-50" : "text-blue-600 bg-blue-50"}`}
                          >
                        ...
                        {comp.attachmentUrls &&
                          comp.attachmentUrls.length > 0 && (
                            <a
                              href={comp.attachmentUrls[0]}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-block mt-2"
                            >
                              <img
                                src={comp.attachmentUrls[0]}
