  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [roomRes, appsRes] = await Promise.all([
        studentRoomApi.get("/me/room", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        studentRoomApi.get("/applications", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
    ...
    return (
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl p-12 text-center border border-gray-200">
        ...
        <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto leading-relaxed">
          {hasPendingApp
            ? "Your application is under review. Wait for approval."
            : "No approved application found. Please apply for a room."}
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href="/student/vacancy"
        ...
        <div className="xl:col-span-2 bg-[#F4F2FF] rounded-3xl p-8 flex flex-col justify-center relative overflow-hidden border border-violet-100 shadow-[0_4px_12px_rgba(0,0,0,0.02)] group">
          ...
          <div className="flex justify-between items-start mb-6 relative z-10">
            <div>
              ...
              <h3 className="text-4xl font-black text-gray-900 mt-4 mb-2 tracking-tight">
                Room {room.roomNumber}
              </h3>
              <p className="text-gray-600 font-medium">
                {roomType} {room.amenities?.[0] ? room.amenities[0] : ""} Room
              </p>
              {room.price > 0 && (
          ...
          <div className="flex gap-4 relative z-10 mt-2 flex-wrap">
            {amenities.slice(0, 3).map((amenity, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 bg-white/60 px-4 py-2 rounded-xl text-sm font-semibold text-gray-700 backdrop-blur-md shadow-sm hover:shadow-md transition-all"
              >
      ...
      <div className="bg-[#F3F8FF]/60 rounded-3xl p-6 lg:p-8 border border-sky-50">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">
            Your Roommates ({roomData.roommates.length})
          </h2>
        </div>
        ...
        {roomData.roommates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {roomData.roommates.map((person, idx) => (
              <div
                key={idx}
                className="bg-white rounded-2xl p-6 flex items-center justify-between shadow-sm border border-white hover:border-sky-100 transition-all group"
              >
