...
const DashboardHome = () => {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      ...
      setError('');
      try {
        const res = await studentRoomApi.get('/notices?limit=6');
  ...
  const displayedNotices = useMemo(() => {
    return (notices || []).slice(0, 6).map((notice) => {
            ...
            displayedNotices.map((card, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className={`p-2.5 rounded-xl ${card.style.bg} ${card.style.text} shadow-sm`}>
                    <Megaphone className="w-4 h-4" />
                  </div>
                </div>

                <h3 className={`text-lg font-bold mb-1.5 ${card.style.text}`}>{card.title}</h3>
                <p className="text-sm font-semibold text-gray-700 mb-2">{card.subtitle}</p>
                <p className="text-xs text-gray-500 font-medium mb-12 leading-relaxed opacity-80">{card.description}</p>
                ...
                <div className="absolute w-full bottom-0 left-0 px-6 pb-6">
                  ...
                  <div className={`w-full h-1.5 border border-white/20 ${card.style.progressBg} rounded-full overflow-hidden`}>
                    <div
            ...
            {[
              ...
              return (
                <div key={idx} className="bg-white/90 hover:bg-white backdrop-blur-xl rounded-2xl p-4 flex items-center justify-between transition-all cursor-pointer shadow-sm hover:shadow-md border border-white hover:border-emerald-100 group">
                  <div>
                    <h4 className="font-bold text-gray-900 group-hover:text-emerald-700 transition-colors">{cat.title}</h4>
